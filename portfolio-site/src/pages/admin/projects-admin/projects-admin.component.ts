...
export class ProjectsAdminComponent {
  ...
  save() {
    ...
    if (this.editMode) {
        ...
        this.projectAdminService.update(this.formModel)
        .subscribe(() => this.loadProjects()); // Güncelleme sonrası projeleri yeniden yükle
            }

        // TODO: API çağrısı -> create
     else {
        console.log('CREATE ->', this.formModel);
        this.projectAdminService.create(this.formModel)
        .subscribe(() => this.loadProjects()); // Yeni proje ekledikten sonra projeleri yeniden yükle
...
// TODO: API çağrısı -> delete
  delete(project: ProjectAdmin) {
    ...
    this.projectAdminService.delete(project.id)
    .subscribe(() => this.loadProjects()); // Silme sonrası projeleri yeniden yükle
